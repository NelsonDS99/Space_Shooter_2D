...
var ammoPingSound := load("res://Scenes/AmmoPing.tscn")

#onready var music := $Music  <--- IGNORE for now... doesn't work

onready var fireDelayTimer := $fireDelayTimer  #reference the timer node for this onready variable
onready var blasterSound = get_node("blasterSound")
...
func _ready():
	...
	Global.points = 0  #reset the points at the start of new game
...
func _on_Hitbox_area_entered(area):
	...
	if area.is_in_group("Enemy") or area.is_in_group("Mine"):
		...
		var ammoPing = ammoPingSound.instance()
		get_parent().add_child(ammoPing)  #add the sound as a child of parent
		ammoPing.play()
